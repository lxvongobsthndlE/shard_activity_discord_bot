...
/** Command: config <option> [<params>]
 *  Configure the bot. Use option "help" to learn more
 */
module.exports = {
    name: 'config',
    description: 'Configure the bot. Use option "help" to learn more.',
    args: true,
    usage: '<option> [params]',
    aliases: [],
    adminOnly: true,
    execute(message, args, guildConfig) {
        if (!guildConfig.ADMIN_IDS.includes(message.author.id)) {
            return message.channel.send(new NoPermissionError(message.author, this.name, args).getEmbed());
        }

        var commandName = args.shift().toLowerCase();

        const command = message.client.configCommands.get(commandName)
            || message.client.configCommands.find(cmd => cmd.aliases && cmd.aliases.includes(commandName));
